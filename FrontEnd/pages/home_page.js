...
const HomeScreen = () => {
	...
	useEffect(() => {
		const fetchUsername = async () => {
		  try {
			const user_uuid = await SecureStore.getItemAsync('user_uuid'); // 저장된 user_uuid 가져오기
			if (user_uuid) {
			  const response = await axios.post('http://61.81.99.111:5000/auth/checkuuid', { user_uuid });
			  if (response.data.StatusCode === 200) {
				setUsername(response.data.data.username);  // username 업데이트
			  } else {
	...
	return (
        <LinearGradient // LinearGradient로 SafeAreaView를 감싸줌
            colors={['#E8DFF5', '#F5FFFA']} // 그라데이션 색상 지정
            style={styles.container} // 기존 컨테이너 스타일 사용
        >
	...
	return (
		<Tab.Navigator
			screenOptions={({ route }) => ({
				...
				headerShown: false, // 상단 헤더를 숨기도록 설정
			})}
		>
			<Tab.Screen
				name="Main"
